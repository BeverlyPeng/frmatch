...
def normalization(adata, cluster_header, scale = True, norm_by = None, filename = None, verbose = 0): 
...
#         print(cluster)
        ...
        if filename: 
            mat.to_csv(filename, index = True)
        del subset
...
def normalization_np(adata, cluster_header, scale = True, norm_by = None, filename = None, verbose = 0): 
    ...
    #     print(cluster, len(keep), keep[:10])
        ...
        ## final re-scaling to 0 and 1
    #     print(np.amax(subset))
    #     print(max(np.amax(subset, axis = 0)))
    #     print(max(np.amax(subset, axis = 1)))
    ...
    if filename: 
        np.to_csv(filename, mat_norm, delimiter = ",")
    adata.X = mat_norm
    if verbose > 0: print("--- %s seconds ---" % (time.time() - start_time))
    return adata
...
def normalization_csr(adata, cluster_header, scale = True, norm_by = None, filename = None): 
