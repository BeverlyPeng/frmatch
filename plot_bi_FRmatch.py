...
def plot_bi_FRmatch(e1_e2, e2_e1, prefix = ["query", "ref"], #name_e1 = "", name_e2 = "", 
    ...
    ## unassigned row
    mat_bi = pd.concat([mat_bi, pd.DataFrame(dict(2*(mat_bi.sum() == 0)), index = [0])])
...
#     mat_bi.columns = [prefix[0] + name_e1 + val for val in pmat_cutoff_e1_e2.columns]
    ...
    if reorder: 
        mat_bi = FRmatch.reorder_FRmatch(mat_bi)
    if two_way_only: 
        mat_bi
    else: 
        ...
        if marker_legend_loc: 
            ...
            a = plt.legend(title = "Marker", handles = handles, bbox_to_anchor = marker_legend_loc) # (1.53, 1)
